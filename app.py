...
# --- 3. CSS & THEME MANAGEMENT (Native Dark Mode) ---
# We remove the complex overrides and stick to standard Streamlit Dark Mode classes.
# This guarantees visibility because we aren't fighting the browser's renderer.

primary_color = "#3b82f6"  # Blue
success_color = "#10b981"  # Green
danger_color = "#ef4444"  # Red
text_color = "#ffffff"  # White text for custom HTML
card_bg = "#1e293b"  # Dark Slate for cards
...
def load_raw_data():
    try:
        ...
        # Normalize Names & Columns to Title Case
...
with st.sidebar:
    ...
    view_mode = st.radio("Select Tool",
                         ["Valuation Dashboard", "Spec-to-Value Matrix", "Smart Deal Evaluator",
                          "Strategic Market Scanner", "Market Intelligence"],
...
if view_mode == "Valuation Dashboard":
    ...
    if st.session_state.valuation_active:
        ...
        with res_c1:
            ...
            fig.update_layout(template="plotly_dark", height=350, xaxis_title="Years from Today")
        ...
        with res_c2:
            ...
            with st.container(border=True):
                ...
                <div style="text-align: center;">
                    <div style="font-size: 0.8rem; opacity: 0.7;">MONTHLY PAYMENT</div>
                    <div style="font-size: 2rem; font-weight: 800; color: {primary_color};">{format_currency(emi)}</div>
                    <div style="font-size: 0.8rem; color: #94a3b8;">Principal: {format_currency(loan_amt)}</div>
                </div>
                """, unsafe_allow_html=True)

# ==============================================================================
# VIEW 2: SPEC-TO-VALUE MATRIX
# ==============================================================================
elif view_mode == "Spec-to-Value Matrix":
    st.markdown('<div class="main-header">Spec-to-Value Matrix</div>', unsafe_allow_html=True)
    st.markdown("""
    <div class="info-box">
    <strong>Market Topography:</strong> Visualize 3D correlations. 
    Darker clusters indicate higher density of listings. Z-Axis is always Price.
    </div>
    """, unsafe_allow_html=True)
    ...
    if df_raw is not None:
        c1, c2, c3 = st.columns(3)
        with c1:
            x_axis = st.selectbox("X-Axis (Feature 1)", ['km_driven', 'max_power', 'mileage', 'engine', 'year'],
                                  index=1, format_func=format_labels)
        with c2:
            y_axis = st.selectbox("Y-Axis (Feature 2)", ['year', 'km_driven', 'mileage', 'engine'], index=0,
                                  format_func=format_labels)
        with c3:
            z_axis = "selling_price"
            color_by = st.selectbox("Color Grouping", ['fuel_type', 'seller_type', 'transmission_type'],
                                    format_func=format_labels)

        if x_axis in df_raw.columns:
            chart_data = df_raw.sample(n=min(2000, len(df_raw)), random_state=42).copy()
            chart_data['short_name'] = chart_data['name'].apply(lambda x: " ".join(str(x).split()[:2]))

            fig = px.scatter_3d(
                chart_data, x=x_axis, y=y_axis, z=z_axis, color=color_by,
                opacity=0.8, size_max=8, hover_name='short_name',
                hover_data={'selling_price': ':.2f'}, template="plotly_dark", height=700,
                title=f"Price Analysis: {format_labels(x_axis)} vs {format_labels(y_axis)}",
                labels={x_axis: format_labels(x_axis), y_axis: format_labels(y_axis), z_axis: "Selling Price",
                        color_by: format_labels(color_by)}
            )
            st.plotly_chart(fig, use_container_width=True)

# ==============================================================================
# VIEW 3: SMART DEAL EVALUATOR
# ==============================================================================
...
# ==============================================================================
# VIEW 4: STRATEGIC MARKET SCANNER (ROBUST LOGIC)
# ==============================================================================
    ...
    st.markdown("""
    <div class="info-box">
    <strong>Advanced Analytical Engine:</strong> 
    Set your budget range below. The AI ranks listings based on <strong>Brand Tier (60%)</strong>, 
    <strong>Budget Utilization (20%)</strong>, and <strong>Asset Condition (20%)</strong>.
    <br><em>This ensures that if you have a high budget, the AI recommends Premium cars (like Mercedes/Toyota) instead of cheap cars (like Alto/WagonR).</em>
    </div>
    """, unsafe_allow_html=True)

    if df_raw is not None:
        # --- SIDEBAR FILTERS (IN-PAGE) ---
        st.markdown('<div class="section-title">SCAN PARAMETERS</div>', unsafe_allow_html=True)
        with st.container(border=True):
            f1, f2, f3, f4 = st.columns(4)
            with f1:
                min_price = st.number_input("Min Budget (₹)", 0, 50000000, 0, step=100000,
                                            help="Set a floor to avoid cheap results.")
                max_price = st.number_input("Max Budget (₹)", 100000, 100000000, 1000000, step=100000)
            with f2:
                max_km = st.number_input("Max Odometer (KM)", 5000, 300000, 80000, step=5000,
                                         help="Filter out high-mileage vehicles for reliability.")
            with f3:
                min_year = st.selectbox("Min Registration Year", [2010, 2012, 2014, 2015, 2017, 2019, 2021], index=0,
                                        help="Filter out older vehicles.")
            with f4:
        ...
        if run_scan:
            with st.spinner("Scanning market database..."):
                time.sleep(0.5)
                ...
                if not filtered.empty:
                    ...
                    tier_norm = filtered['tier_score'] / 3.0
                    budget_util_norm = filtered['selling_price'] / max_price
                    year_norm = (filtered['year'] - filtered['year'].min()) / (
                                filtered['year'].max() - filtered['year'].min() + 0.1)

                    # WEIGHTED SCORE
                    filtered['value_score'] = (tier_norm * 0.6) + (budget_util_norm * 0.2) + (year_norm * 0.2)
                    filtered['value_score'] = (filtered['value_score'] * 100).astype(int)

                    filtered = filtered.sort_values('value_score', ascending=False)
                    top_picks = filtered.head(3)

                    st.markdown("<br>", unsafe_allow_html=True)
                    st.markdown("**🏆 Top 3 Premium Opportunities**")
                    ...
                    for idx, row in enumerate(top_picks.itertuples()):
                        with cols[idx]:
                            st.markdown(f"""
                            <div class="deal-card">
                                <div class="score-badge">{row.value_score}</div>
                                ...
                                <div class="deal-specs">
                                    ...
                                    <span>🛣️ {int(row.km_driven):,} km</span>
                    ...
                    st.markdown("<br>", unsafe_allow_html=True)
                    st.markdown('<div class="section-title">FULL MARKET LEDGER</div>', unsafe_allow_html=True)

                    display_df = filtered[
                        ['name', 'selling_price', 'year', 'km_driven', 'fuel_type', 'transmission_type',
                         'value_score']].copy()
                    display_df.columns = ['Vehicle Name', 'Price', 'Year', 'Odometer', 'Fuel', 'Transmission',
                                          'Value Score']
                    ...
                    st.warning("No vehicles match your strict criteria. Try widening your price range.")

# ==============================================================================
# VIEW 5: MARKET INTELLIGENCE
# ==============================================================================
